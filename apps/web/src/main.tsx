...
function App(){const[user,setUser]=useState<User>(viewer),[csrf,setCsrf]=useState(''),[data,setData]=useState<Data|null>(null),[error,setError]=useState(''),[modal,setModal]=useState<string|null>(()=>{const p=new URLSearchParams(window.location.search);return p.get('username')&&p.get('password')?'login':null}),[busy,setBusy]=useState(true),[showHiddenMatches,setShowHiddenMatches]=useState(false),[auditRevision,setAuditRevision]=useState(0);
 ...
 return <div className="app"><header><div className="brand"><span className="brand-mark small">3C</span><div><b>CUE ARENA</b><small>TOURNAMENT SERIES</small></div></div><nav><a className="active">Giải đấu</a><a href="#history">Lịch sử</a>{user.role!=='viewer'&&<a href="#vote-history">Bình chọn</a>}{user.role==='admin'&&<a href="#admin">Quản trị</a>}</nav>{user.role==='viewer'?<div className="account viewer-account"><span><b>Khách</b><small>Chỉ xem</small></span><button className="secondary dark" onClick={()=>setModal('login')}>Đăng nhập</button></div>:<div className="account"><span><b>{user.displayName}</b><small>{user.role==='admin'?'Quản trị viên':'Thành viên'}</small></span><span className="avatar">{user.displayName.charAt(0)}</span><button className="icon" title="Hồ sơ cá nhân" onClick={()=>setModal('profile')}><UserCog size={18}/></button><button className="icon" title="Đăng xuất" onClick={()=>request('/auth/logout',{method:'POST'}).finally(()=>{setUser(viewer);setCsrf('')})}><LogOut size={18}/></button></div>}</header>
 <main><section className="hero"><div className="eyebrow"><span></span> {tournament?.status==='finished'?'ĐÃ KẾT THÚC':tournamentMatches.length?'ĐANG DIỄN RA':'CHỜ XẾP LỊCH'}</div><h1>{tournament?.name||'Chưa có giải đấu'}</h1><p>Đấu loại trực tiếp · {activeTeams.length} đội · Một nhà vô địch</p><div className="hero-actions"><button className="primary" onClick={()=>document.querySelector('#matches')?.scrollIntoView({behavior:'smooth'})}>Xem danh sách đấu <Trophy size={17}/></button>{user.role==='admin'&&tournament&&tournamentMatches.length===0&&<button className="secondary" onClick={()=>action(`/tournaments/${tournament.id}/generate`,'POST')}><Sparkles size={17}/> Tự động xếp lịch</button>}{user.role==='admin'&&tournament&&tournamentMatches.length>0&&<><button className="secondary" disabled={finishedMatchCount===0} title={finishedMatchCount?'Chỉ lưu trận đã kết thúc và bỏ qua các trận còn lại':'Chưa có trận đã kết thúc; hãy dùng Huỷ giải đấu'} onClick={()=>{const completed=tournament.status==='finished',message=completed?`Lưu ${finishedMatchCount} trận đã kết thúc cùng log vote và dọn lịch đấu hiện tại?`:`Kết thúc giải sớm sẽ chỉ lưu ${finishedMatchCount} trận đã kết thúc cùng log vote đã chấm. Các trận chưa đấu/không xảy ra và vote của chúng sẽ bị bỏ qua. Tiếp tục?`;if(window.confirm(message))action(`/tournaments/${tournament.id}/finish-early`,'POST')}}><Flag size={17}/> {tournament.status==='finished'?'Lưu và làm mới giải':'Kết thúc giải đấu sớm'}</button><button className="danger hero-danger" onClick={()=>{if(window.confirm('Huỷ giải sẽ xóa toàn bộ lịch đấu và bình chọn hiện tại mà không lưu lịch sử. Thao tác này không thể hoàn tác. Tiếp tục?'))action(`/tournaments/${tournament.id}/cancel`,'POST')}}><Trash2 size={17}/> Huỷ giải đấu</button></>}</div><div className="ball ball1">3</div><div className="ball ball2">C</div></section>
 {error&&<div className="toast" onClick={()=>setError('')}><X size={16}/>{error}</div>}
 <section className="stats"><div><Trophy/><span><b>{publicMatches.filter(m=>m.status==='finished').length}</b> trận đã đấu</span></div><div><CalendarDays/><span><b>{publicMatches.filter(m=>m.status==='scheduled').length}</b> trận sắp tới</span></div><div><Users/><span><b>{activeTeams.length}</b> đội tham gia</span></div><div><Vote/><span><b>{publicMatches.reduce((n,m)=>n+m.votes_a+m.votes_b,0)}</b> lượt dự đoán</span></div></section>
 <section id="matches" className="section matches-section"><div className="section-head matches-section-head"><div className="section-title"><span className="kicker">ĐƯỜNG ĐẾN CHỨC VÔ ĐỊCH</span><h2>Danh sách trận đấu</h2></div><div className="match-list-tools"><div className="legend"><i className="live"></i>Sắp đấu <i className="done"></i>Đã kết thúc</div>{user.role==='user'&&<div className="vote-policy"><LockKeyhole size={15}/> Vote cần xác nhận · Không thể tự huỷ</div>}{user.role==='admin'&&tournamentMatches.length>0&&<div className="match-list-actions">{hiddenMatchCount>0&&<button className="secondary dark" onClick={()=>setShowHiddenMatches(value=>!value)}>{showHiddenMatches?<EyeOff size={17}/>:<Eye size={17}/>} {showHiddenMatches?'Ẩn trận đã ẩn':`Hiện ${hiddenMatchCount} trận đã ẩn`}</button>}<button className="secondary dark" disabled={!canReorder} title={canReorder?'Kéo thả để đổi cặp vòng đầu':'Không thể đổi cặp sau khi đã có vote hoặc kết quả'} onClick={()=>setModal('pairings')}><GripVertical size={17}/> Sắp xếp cặp đấu</button></div>}</div></div>{matches.length?<MatchesTable matches={matches} maxRound={maxRound} isAdmin={user.role==='admin'} canVote={user.role==='user'} showHandicap={user.role!=='viewer'} onVote={vote} onEdit={m=>setModal('match:'+m.id)} onStats={m=>setModal('votes:'+m.id)} onLock={m=>action(`/matches/${m.id}`,'PATCH',{votingLocked:!m.voting_locked})} onVisibility={m=>action(`/matches/${m.id}`,'PATCH',{hidden:!m.hidden})}/>:<div className="empty"><Trophy/><h3>{hiddenMatchCount?'Các trận đấu đang được ẩn':'Lịch đấu chưa được xếp'}</h3><p>{hiddenMatchCount?'Admin có thể dùng nút phía trên để xem và khôi phục trận đã ẩn.':'Admin có thể tạo lịch tự động sau khi chốt danh sách đội.'}</p></div>}</section>
 <Leaderboard key={tournamentMatches.map(m=>m.winner_id||0).join(',')} request={request} isAdmin={user.role==='admin'}/>
 <MatchHistory history={data?.history||[]}/>
 {user.role!=='viewer'&&<VoteHistory key={auditRevision} request={request} isAdmin={user.role==='admin'}/>}
 ...
 {modal==='pairings'&&tournament&&<PairingModal matches={tournamentMatches} onClose={()=>setModal(null)} onSubmit={teamIds=>action(`/tournaments/${tournament.id}/pairings`,'PATCH',{teamIds})}/>}
 {modal?.startsWith('match:')&&<MatchModal match={tournamentMatches.find(m=>m.id===Number(modal.split(':')[1]))!} onClose={()=>setModal(null)} onSubmit={v=>action(`/matches/${modal.split(':')[1]}`,'PATCH',v)}/>}
